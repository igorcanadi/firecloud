...
def take_server_down(host, port=None):
  ...
  command = "killall server 2>/dev/null"
...
def bring_server_up(host, port):
  ...
  hl = map( lambda t: ':'.join(t), server_list)
	# ssh -i 739key user739@$ip "killall servers; ./server $port 1 1 1000 10 R < /dev/null > error_log 2>&1 &"
  command = "./server {0} 1 1 1000 10 R < /dev/null > error_log 2>&1".format(port)
  remote_exec((host, 22), command)
...
def hard_reset():
  ...
  for host, port in server_list:
    ...
    # for venkat's backups
    remote_exec((host, 22), "rm -rf backup*")
  sleep(RESET_DELAY)
  for host, port in server_list:
    flush_iptables(host)
  sleep(RESET_DELAY)
